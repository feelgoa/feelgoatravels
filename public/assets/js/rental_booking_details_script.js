...
function checktime(pickup_time, message, error_message) {
    var msg = document.getElementById(error_message);
    var time = pickup_time.value;
    //Divide into substring
    if (time.length == 7) {
        var hour = time.substring(0, 2);
        var minute = time.substring(3, 5);
        var type = time.substring(5, 7);
    } else {
        var hour = time.substring(0, 1);
        var minute = time.substring(2, 4);
        var type = time.substring(4, 6);
    }
    ...
    if (type == "pm") {
        if (hour == 12 || hour <= 9) {
            if (hour == 9) {
                if (minute > 0) {
                    pickup_time.style.borderColor = "red";
                    msg.style.display = "inline-block";
                    msg.innerHTML = message;
                    pickup_time.focus();
                    document.getElementById("submit_btn").disabled = true;
                    ...
                    pickup_time.style.borderColor = "#66afe9";
                    msg.style.display = "none";
                    document.getElementById("submit_btn").disabled = false;
                    return true;
                }
