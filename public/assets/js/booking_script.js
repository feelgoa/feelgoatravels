...
$(".tours").click(function() {
	if ($("#customCheckBox5").prop("checked") == true) {
...
function ValidateEmptyNumberField(cont, message) {
	var msg = document.getElementById("error_message");
	var contformat = /^\d+$/;
	if (cont.value != "") {
		if ((cont.id =='age') & (cont.value == "0")) {
			cont.style.borderColor = "red";
			msg.style.display = "inline-block";
			msg.innerHTML = 'Age cannot be zero';
			cont.focus();
			return false;
		}
...
function personal_details_check() {
	...
	var male_cnt = document.getElementById("male_count");
	var female_cnt = document.getElementById("female_count");
	if (name.value == "" || email.value == "" || contact.value=="" || (male.checked==false && female.checked==false) || age.value=="" || place.value=="" ) {
		...
		if (parseInt(male_cnt.value) + parseInt(female_cnt.value) == 0) {
			msg.style.display = "inline-block";
			msg.innerHTML = "Count of both male and female cannot be empty.";
			return false;
		} else {
            msg.style.display = "none";
			return true;
