...
class UserController extends Controller
{
	...
	function getbookingstatusdetails(Request $request) {
		...
		$booking_details['booking_travel_dates'] = DB::select('SELECT * FROM booking_spot where booking_id = "'.$booking_id.'"');
		return view('user.booking_status_view',['title'=> BOOKING_STATUS_TITLE,'pnrno'=>$get_booking_id ['booking'][0]['booking_pnr'],'details'=>$booking_details]);
	}
